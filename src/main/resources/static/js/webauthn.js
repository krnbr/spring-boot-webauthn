...
(() => {
    ...
    async function authenticateOrError(headers, contextPath, useConditionalMediation) {
        try {
            window.location.href = await webauthn_core_default.authenticate(headers, contextPath, useConditionalMediation);
        } catch (err) {
            console.error(err);
    ...
    function setError(ui, msg) {
        ...
        const errorMessage = ui.getErrorMessage();
        if (!error) {
            return;
        }
        errorMessage.textContent = message;
        setVisibility(error, 'grid');
    }
    function setSuccess(ui, msg) {
        const message = msg || 'Success!';
        resetPopups(ui);
        const success = ui.getSuccess();
        const successMessage = ui.getSuccessMessage();
        if (!success) {
        ...
        setVisibility(success, 'grid');
    }
    function resetPopups(ui) {
        const success = ui.getSuccess();
        const error = ui.getError();
        setVisibility(success, 'none');
    ...
    function initActions(headers, contextPath, ui) {
        ui.closeSuccess().addEventListener('click', function(e) {
            setVisibility(ui.getSuccess(), 'none');
        });
        ui.closeError().addEventListener('click', function(e) {
            setVisibility(ui.getError(), 'none');
        });
        ui.getDeleteForms().forEach(
            (form) => form.addEventListener("submit", async function(e) {
                ...
                try {
                    ...
                    // instead of referring to location remove the row using javascript
                    // window.location.href = `${contextPath}/?success`;
                    const row = this.closest('tr');
                    if (row) {
                        setSuccess(ui, "Successfully deleted passkey!");
                        row.remove();
                    }
                } catch (err) {
    ...
    async function setupRegistration(headers, contextPath, ui) {
        ...
        initActions(headers, contextPath, ui);
        ui.getRegisterButton().addEventListener("click", async () => {
            ...
            try {
                ...
                setSuccess(ui, "The passkey labelled - "+ label + " registered successfully.");
                await htmx.ajax('GET', `/passkeys`, {
                    target: '#passkeys-container',
                    swap: 'innerHTML'
                }).then(() => {
                    // reinitialize the UI common actions
                    console.log("refreshed passkeys !!!");
                    initActions(headers, contextPath, ui);
                });
            } catch (err) {
                setError(ui, err.message);
